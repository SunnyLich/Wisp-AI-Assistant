...
Object.assign(I18N.reg['es'].tr, {
  ...
  "Wisp offers three mutually exclusive settings under Settings → App → Privacy mode:": "Wisp ofrece tres opciones mutuamente excluyentes en <strong>Configuración → Aplicación → Modo de privacidad</strong>:",
  "Mode": "Modo",
...
Object.assign(I18N.reg['es'].meta, {
  "live-agents": {
    ...
    sub: "Ejecuta sesiones completas de agentes locales detrás de Wisp, con progreso en vivo, aprobaciones y continuidad reanudable.",
  },
});
Object.assign(I18N.reg['es'].tr, {
  "Wisp": "Wisp",
  ...
  "One Wisp interface": "Una sola interfaz de Wisp",
  "Hotkey queries and full-chat prompts use the selected engine. Wisp still captures context, applies privacy controls, displays the stream, and stores a local copy of the conversation.": "Las consultas por atajo y los prompts del chat completo usan el motor seleccionado. Wisp sigue capturando contexto, aplicando los controles de privacidad, mostrando la transmisión y guardando una copia local de la conversación.",
  "Select and sign in": "Seleccionar e iniciar sesión",
  "Open Settings → App → Run conversations with and choose Wisp, ChatGPT, or Claude Agent. When an agent is selected, the same card shows its login status and provides Sign in, Sign out, and Refresh actions. Sign-in opens the provider's terminal and browser flow.": "Abre <strong>Configuración → Aplicación → Ejecutar conversaciones con</strong> y elige Wisp, ChatGPT o Claude Agent. Al seleccionar un agente, la misma tarjeta muestra el estado de inicio de sesión y ofrece las acciones <strong>Iniciar sesión</strong>, <strong>Cerrar sesión</strong> y <strong>Actualizar</strong>. El inicio de sesión abre el flujo de terminal y navegador del proveedor.",
  "ChatGPT mode requires the Codex CLI. Wisp gives Codex an isolated local profile, so its login and resumable Wisp sessions do not appear in your personal Codex history. Claude Agent uses the bundled Claude Agent SDK when available and authenticates through the Claude Code CLI.": "El modo ChatGPT requiere la CLI de Codex. Wisp proporciona a Codex un perfil local aislado, por lo que su inicio de sesión y las sesiones reanudables de Wisp no aparecen en tu historial personal de Codex. Claude Agent usa el SDK de Claude Agent incluido cuando está disponible y se autentica mediante la CLI de Claude Code.",
  ...
  "Best for": "Ideal para",
  "Wisp sends the complete local Wisp history on every request. Each request starts without a retained provider continuation link.": "Wisp envía todo el historial local de Wisp en cada solicitud. Cada solicitud empieza sin conservar un enlace de continuación del proveedor.",
  ...
  "Wisp transfers the existing history once, stores the returned provider session ID, and resumes that session for later prompts.": "Wisp transfiere una vez el historial existente, guarda el ID de sesión devuelto por el proveedor y reanuda esa sesión en los prompts posteriores.",
  ...
  "Wisp keeps separate Wisp, ChatGPT/Codex, and Claude conversation namespaces so switching engines does not append a prompt to the wrong provider's history. Changing the project folder also starts a clean provider session.": "Wisp mantiene separados los espacios de conversación de Wisp, ChatGPT/Codex y Claude para que cambiar de motor no añada un prompt al historial del proveedor equivocado. Cambiar la carpeta del proyecto también inicia una sesión limpia del proveedor.",
  "What appears while the agent works": "Qué aparece mientras trabaja el agente",
  "Replies stream normally, while visible reasoning summaries, plans, tool starts, command or file progress, status updates, and approval requests appear as progress in the bubble and chat. Wisp can only show events the provider exposes; private hidden chain-of-thought is never available.": "Las respuestas se transmiten normalmente, mientras que los resúmenes de razonamiento visibles, planes, inicios de herramientas, progreso de comandos o archivos, estados y solicitudes de aprobación aparecen como progreso en la burbuja y el chat. Wisp solo puede mostrar los eventos que expone el proveedor; la cadena de pensamiento privada y oculta nunca está disponible.",
  "When a command, file change, or other protected tool needs permission, Wisp presents the action, path, command, or diff it received and sends your decision back to the running agent.": "Cuando un comando, un cambio de archivo u otra herramienta protegida necesita permiso, Wisp muestra la acción, ruta, comando o diff recibido y devuelve tu decisión al agente en ejecución.",
  "Live provider controls": "Controles en vivo del proveedor",
  "When ChatGPT or Claude Agent is active, a provider badge appears below the floating Wisp icon. Click it to change settings for the next turn:": "Cuando ChatGPT o Claude Agent está activo, aparece una insignia del proveedor bajo el icono flotante de Wisp. Haz clic en ella para cambiar los ajustes del siguiente turno:",
  "Control": "Control",
  "Effect": "Efecto",
