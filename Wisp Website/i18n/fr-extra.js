...
Object.assign(I18N.reg['fr'].tr, {
  ...
  "Wisp offers three mutually exclusive settings under Settings → App → Privacy mode:": "Wisp propose trois options mutuellement exclusives dans <strong>Paramètres → Application → Mode de confidentialité</strong> :",
  "Mode": "Mode",
  ...
  "Wisp loads the advanced model into memory and warms it in the background when Wisp starts or after you enable Advanced mode. Warm-up may take tens of seconds on a CPU. If you send a request before it finishes, that request waits; later scans reuse the loaded model and are faster.": "Wisp charge le modèle avancé en mémoire et le préchauffe en arrière-plan au démarrage ou après l'activation du mode avancé. Le préchauffage peut prendre plusieurs dizaines de secondes sur un processeur. Si vous envoyez une requête avant la fin, elle attend ; les analyses suivantes réutilisent le modèle chargé et sont plus rapides.",
  "Wisp merges the AI model's findings with the built-in detector, replaces sensitive spans with stable session placeholders such as [PERSON_1], and can show a review before the request is sent. It then checks the redacted text again. If the model is unavailable, detection fails, or sensitive text remains, Wisp blocks the cloud send.": "Wisp fusionne les résultats du modèle d'IA avec ceux du détecteur intégré, remplace les passages sensibles par des espaces réservés de session stables comme <code>[PERSON_1]</code> et peut afficher une vérification avant l'envoi de la requête. Il contrôle ensuite à nouveau le texte expurgé. Si le modèle est indisponible, si la détection échoue ou s'il reste du texte sensible, Wisp bloque l'envoi vers le cloud.",
...
Object.assign(I18N.reg['fr'].meta, {
  "live-agents": {
    ...
    sub: "Exécutez des sessions complètes d’agents locaux derrière Wisp, avec progression en direct, approbations et continuité reprenable.",
  },
});
Object.assign(I18N.reg['fr'].tr, {
  "Wisp": "Wisp",
  ...
  "Wisp can use more than a model API. It can run a complete local coding-agent harness behind both the overlay and the full chat window. The ChatGPT option starts the installed Codex CLI in app-server mode; Claude Agent uses the Claude Agent SDK and the same Claude Code CLI authentication. Wisp supplies the prompt and captured context, then streams the agent's visible output back into the normal Wisp interface.": "Wisp peut faire plus qu’utiliser une API de modèle. Il peut exécuter un agent de programmation local complet derrière la superposition comme la fenêtre de chat. L’option <strong>ChatGPT</strong> démarre la CLI Codex installée en mode app-server ; <strong>Claude Agent</strong> utilise le SDK Claude Agent et la même authentification que la CLI Claude Code. Wisp fournit le prompt et le contexte capturé, puis diffuse la sortie visible de l’agent dans l’interface Wisp habituelle.",
  "Run conversations with": "Exécuter les conversations avec",
  "Behavior": "Comportement",
  "Uses the LLM provider and model configured in Wisp.": "Utilise le fournisseur LLM et le modèle configurés dans Wisp.",
  ...
  "One Wisp interface": "Une seule interface Wisp",
  "Hotkey queries and full-chat prompts use the selected engine. Wisp still captures context, applies privacy controls, displays the stream, and stores a local copy of the conversation.": "Les requêtes par raccourci et les prompts du chat complet utilisent le moteur sélectionné. Wisp continue de capturer le contexte, d’appliquer les contrôles de confidentialité, d’afficher le flux et de conserver une copie locale de la conversation.",
  "Select and sign in": "Sélectionner et se connecter",
  "Open Settings → App → Run conversations with and choose Wisp, ChatGPT, or Claude Agent. When an agent is selected, the same card shows its login status and provides Sign in, Sign out, and Refresh actions. Sign-in opens the provider's terminal and browser flow.": "Ouvrez <strong>Paramètres → Application → Exécuter les conversations avec</strong>, puis choisissez Wisp, ChatGPT ou Claude Agent. Lorsqu’un agent est sélectionné, la même carte affiche l’état de connexion et propose les actions <strong>Se connecter</strong>, <strong>Se déconnecter</strong> et <strong>Actualiser</strong>. La connexion ouvre le parcours terminal et navigateur du fournisseur.",
  "ChatGPT mode requires the Codex CLI. Wisp gives Codex an isolated local profile, so its login and resumable Wisp sessions do not appear in your personal Codex history. Claude Agent uses the bundled Claude Agent SDK when available and authenticates through the Claude Code CLI.": "Le mode ChatGPT nécessite la CLI Codex. Wisp attribue à Codex un profil local isolé : sa connexion et les sessions Wisp reprenables n’apparaissent donc pas dans votre historique Codex personnel. Claude Agent utilise le SDK Claude Agent inclus lorsqu’il est disponible et s’authentifie via la CLI Claude Code.",
  ...
  "Best for": "Idéal pour",
  "Wisp sends the complete local Wisp history on every request. Each request starts without a retained provider continuation link.": "Wisp envoie tout l’historique Wisp local à chaque requête. Chaque requête démarre sans conserver de lien de continuation avec le fournisseur.",
  ...
  "Wisp transfers the existing history once, stores the returned provider session ID, and resumes that session for later prompts.": "Wisp transfère une fois l’historique existant, enregistre l’identifiant de session renvoyé par le fournisseur et reprend cette session pour les prompts suivants.",
  ...
  "Wisp keeps separate Wisp, ChatGPT/Codex, and Claude conversation namespaces so switching engines does not append a prompt to the wrong provider's history. Changing the project folder also starts a clean provider session.": "Wisp sépare les espaces de conversation Wisp, ChatGPT/Codex et Claude afin qu’un changement de moteur n’ajoute pas un prompt à l’historique du mauvais fournisseur. Changer de dossier de projet démarre également une nouvelle session fournisseur.",
  "What appears while the agent works": "Ce qui apparaît pendant le travail de l’agent",
  "Replies stream normally, while visible reasoning summaries, plans, tool starts, command or file progress, status updates, and approval requests appear as progress in the bubble and chat. Wisp can only show events the provider exposes; private hidden chain-of-thought is never available.": "Les réponses sont diffusées normalement, tandis que les résumés de raisonnement visibles, plans, lancements d’outils, progressions de commandes ou de fichiers, états et demandes d’approbation apparaissent comme progression dans la bulle et le chat. Wisp ne peut afficher que les événements exposés par le fournisseur ; la chaîne de pensée privée et masquée n’est jamais disponible.",
  "When a command, file change, or other protected tool needs permission, Wisp presents the action, path, command, or diff it received and sends your decision back to the running agent.": "Lorsqu’une commande, une modification de fichier ou un autre outil protégé nécessite une autorisation, Wisp présente l’action, le chemin, la commande ou le diff reçu, puis renvoie votre décision à l’agent en cours d’exécution.",
  "Live provider controls": "Contrôles du fournisseur en direct",
  "When ChatGPT or Claude Agent is active, a provider badge appears below the floating Wisp icon. Click it to change settings for the next turn:": "Lorsque ChatGPT ou Claude Agent est actif, un badge du fournisseur apparaît sous l’icône Wisp flottante. Cliquez dessus pour modifier les réglages du prochain tour :",
  "Control": "Contrôle",
  "Effect": "Effet",
