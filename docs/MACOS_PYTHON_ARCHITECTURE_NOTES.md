# Pure-Python Worker Architecture Notes

This file records how the pure-Python worker target uses older macOS code as
evidence where it still matters. Old patches are not automatically ported. Each
subsystem must classify old behavior before implementation.
...
|---|---|---|---|
| Protocol stdout | `core.macos_helper.protocol`, `runtime/brain/wisp_brain/protocol.py` | Keep as invariant | All workers use newline-delimited JSON and reserve stdout for protocol messages. |
| Worker isolation | `core.macos_helper.client`, `core.macos_helper.host`, `runtime/brain/wisp_brain/host.py` | Keep as invariant | Supervisor owns worker lifecycle, timeouts, stderr capture, and crash recovery. |
| UI main thread | legacy single-process app, `macos/ui_host/wisp_qt_ui_host.py` | Keep as invariant | `wisp-ui` is the only PySide process; all widget work happens on the Qt main thread. |
...
| Brain behavior | `runtime/brain/wisp_brain`, `core/query_pipeline.py` | Keep as invariant | Reuse the existing headless brain worker; do not import UI/native/audio stacks. |
| Product flows | legacy single-process hotkey, query, rewrite, snip, voice, context, and settings behavior | Keep as invariant; replace structure | `runtime.supervisor.flows.FlowController` owns routing: hotkey -> intent -> native context -> brain stream -> UI reply -> optional audio, plus rewrite/pasteback, snip, voice, chat, memory, and settings reload. |
| Drag/drop and buffered context | legacy overlay flow, `ui/overlay.py`, `ui/drop_zone.py` | Keep as invariant | Dropped/buffered context is collected by UI/native, consumed once by the supervisor, summarized in UI, and sent to `wisp-brain` as prompt context. |
| Chat window LLM calls | `ui/chat_window.py`, legacy single-process chat flow | Replace with clean structure | `wisp-ui` keeps the existing chat widget, but its `send_fn` emits `ui.chat.request`; supervisor streams `brain.chat` chunks back through UI methods. |
...
| Agent task/history | `ui/agent/task_window.py`, `core.agent`, `runtime/brain/wisp_brain` | Replace with clean structure | Task collection reuses the shared Qt spec dialog, but run/history/cancel/approval behavior is protocol-backed: `wisp-ui` emits task/history intents, the supervisor streams `brain.agent.*`, and only `wisp-brain` runs `core.agent`. |
| Addon manager | `ui/addon_manager.py`, `core.addon_manager`, `runtime/brain/wisp_brain` | Replace with clean structure | Query-time addon hooks and addon-manager list/action behavior run in `wisp-brain`; the Python UI shows a protocol-fed addon dialog instead of importing `core.addon_manager`. |

## Current Implementation Status

- `runtime.supervisor` provides worker lifecycle and ID-correlated IPC.
- `runtime.workers.native_host` exposes permissions, hotkey start/stop, context, capture, clipboard, pasteback, and settings-open methods.
- `runtime.workers.ui_host` owns PySide UI windows and response surfaces through protocol methods.
- `runtime.workers.brain_host` wraps the existing headless brain host.
- `runtime.workers.audio_host` owns audio/STT/TTS imports and playback.
- `runtime.supervisor.flows` wires feature flows across workers and contains
...
- Stable TCC identity for packaged Python builds.
