...
- **334 / 472** functions are accepted through a production entry point with a successful observable result.
- **36 / 472** have a name-matched candidate test that still needs code-path audit.
- **102 / 472** are untested at the real-entry acceptance level.
- **319 / 472** have completed dependency audits.
- **121 / 121** declared A -> B interaction matrices are accepted. These include the earlier launcher, Settings, appearance, platform, local-file, onboarding, and provider matrices, plus planned chunking, automatic elaboration, external transcript synchronization/ownership, every provider-control option, authentication, connection/model management, every supported provider route, Chat/Image/Memory row operations, route failover/cooldown, provider adaptation, and capability-warning relationships.
