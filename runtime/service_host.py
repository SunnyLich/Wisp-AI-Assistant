...
def run_host(
    *,
    ...
    def send(obj: dict[str, Any]) -> None:
        """Write a message to the peer over the protocol pipe (thread-safe)."""
        with write_lock:
            try:
    ...
    def emit(event: str, data: Any = None, req_id: Any = None) -> None:
        """Send an event message to the peer."""
    ...
    def ping(value: Any = None) -> dict[str, Any]:
        """Handle ping for runtime service host."""
        return {
            "pong": True,
            "value": value,
            "pid": os.getpid(),
    ...
    def respond(req_id: Any, ok: bool, *, result: Any = None, error: str | None = None) -> None:
        """Handle respond for runtime service host."""
