...
full session, the way the Swift ``BrainClient`` would in real use.

This is the "try them together" pass. It spawns the actual sidecar process and
...
talks to the sidecar exactly like the Phase-1 handshake test does.
"""
...
def test_full_session_like_real_use(sidecar, tmp_path):
    s = sidecar
    ...
    # 7. A final ping proves the sidecar stayed responsive through all of it.
    _, ping2 = s.call("ping")
    assert ping2["pong"] is True


def test_cancel_stops_a_long_stream(sidecar):
    s = sidecar
    rid = next(s._ids)
    ev = threading.Event()
...
def test_unknown_method_is_an_error(sidecar):
    with pytest.raises(RuntimeError):
        sidecar.call("does.not.exist", timeout=5)
