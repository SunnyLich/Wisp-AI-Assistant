...
选中文本，按下常规快捷键，点击一个意图键，Wisp 就会使用您启用的上下文来源向您配置的模型提问。回复将以流式方式显示在浮动图标旁边的紧凑气泡中。如果启用了 TTS，答案会在到达时同步朗读。
...
| 场景 | 操作 | 结果 |
| --- | --- | --- |
| 需要解释选中文本 | 选中文本，按下常规快捷键，然后选择 `W`（这是什么？）或 `A`（简单解释） | Wisp 在悬浮窗中解释选中内容 |
| 需要重写一个句子 | 先选中句子，按下重写快捷键，然后选择 `W`、`A` 或 `D` 进行语法、简化或语气调整 | Wisp 重写选中文本并可粘贴回去 |
| 需要提出自己的问题 | 按下常规快捷键，按 `S`，输入提示词，然后按 Enter | Wisp 使用该调用者启用的上下文发送您的自定义提示词 |
...
使用设置窗口进行常规设置。它可以存储提供商密钥、选择模型路由、配置语音、运行设置检查、解释缺失的可选功能，并显示不支持的模型功能的警告。提供方密钥和 OAuth token 会存储在操作系统密钥链中：Windows 凭据管理器、macOS 钥匙串，或 Linux 上的 Secret Service/KWallet，而不是明文配置文件。
...
| 快捷键 | 操作 |
| --- | --- |
| Windows：`Ctrl+Q`；macOS/Linux：`Ctrl+Alt+Space` | 打开常规意图选择器 |
| Windows：`Ctrl+Shift+Q`；macOS/Linux：`Ctrl+Alt+Shift+Space` | 打开重写/粘贴意图选择器 |
...
- 提供方密钥和 OAuth token 会存储在您的操作系统密钥链中，也就是 Windows、macOS 或 Linux 桌面内置的安全密码存储区。
...
*macOS 打包版距离上次真实机器实测已经有一段时间，因此可能比 Windows 版本或仓库启动器路径更容易出问题。如果遇到问题，请尝试使用 `Start Wisp.command` 运行仓库版本；这是目前支持最好的 macOS 路径。租用 Apple 硬件进行新的测试需要费用，所以如果你想支持更多 macOS 验证，可以在 [Buy Me a Coffee](https://buymeacoffee.com/sunnylich) 捐助。当然完全没有压力：附带日志的清晰 bug 报告也非常有帮助。
