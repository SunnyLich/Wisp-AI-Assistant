"""
chunks.py — the ContextChunk model plus a realistic seed corpus.

A ContextChunk is one retrievable unit of stored memory/project knowledge. The
seed set below mirrors the plan's Day-1 ask: ~30 chunks drawn from this user's
real themes (Git confusion, PySide6, the Wisp/overlay app, Supabase/CORS, the
AI-team workflow, Windows/Linux env issues) plus a few intentionally generic
ones so the router has irrelevant material to *avoid*.

Chunks carry their own extracted terms/phrases/identifiers so the index can be
built once and queries scored fast. Call ``ContextChunk.from_text`` to build a
chunk and have extraction run automatically.
"""
...
_SEED: list[tuple[str, str, str, float, float]] = [
    ...
    # --- generic / non-project (router should AVOID these) ----------------
    ("generic-markup-900",
...
def load_seed_chunks() -> list[ContextChunk]:
    """Build the in-memory seed corpus."""
    return [
