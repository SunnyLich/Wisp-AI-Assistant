...
class RetrievalIndexes:
    ...
    def term_idf(self, term: str) -> float:
        ...
        if df == 0:
            # Unseen in corpus: treat as moderately rare, not maximal, since we
            # have no chunk to match it against anyway.
    ...
    def identifier_idf(self, ident: str) -> float:
        ...
        if df == 0:
            return 0.0  # no chunk has it -> no evidence
        return math.log((self.n_chunks + 1) / (df + 1)) + 1.0  # floor so any hit counts
